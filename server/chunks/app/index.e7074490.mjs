      ...
      contract.methods.getSaleNFTList(true, true).call({ from: account.value }).then(async (value) => {
        if (value.length > 0) {
          const getResult = value.map(async (item, index2) => {
            const { data } = await useAsyncData(index2.toString(), () => $fetch(item.tokenURI), "$RT56HID8WY");
            const tempItem = {
              ...item,
              ...{ metadata: data.value }
            };
...
const index = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-6e845cf6"]]);

export { index as default };
//# sourceMappingURL=index.e7074490.mjs.map
