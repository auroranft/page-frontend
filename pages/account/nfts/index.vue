...
function getNftsList() {
  ...
  contract.methods.getNFTListByOwner()
    ...
    <section class="py-5 text-center container">
      <div class="row py-lg-5">
        <div class="col-lg-6 col-md-8 mx-auto">
          <h1 class="fw-light">My NFTs</h1>
          <p class="lead text-muted text-truncate">{{ account }}</p>
          <p>
            <button :class="`btn my-2 ${isCopy ? 'btn-success' : 'btn-primary' }`" @click="copyAddress(account, $event)">
        ...
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-4 g-4">
          <div class="col" v-for="item in nftsList" :key="item.nftId">
            <div class="card shadow-sm">
              <div class="card-body">
                <p class="card-text">
                  <NuxtLink :to="`/account/nfts/${item.collection.token}`">
